...
class _RenderProgressBar extends RenderBox {
  ...
  TextPainter _layoutText(String text) {
    TextPainter textPainter = TextPainter(
      text: TextSpan(
          text: text, style: TextStyle(color: floatingBtnColor)
      ),
  ...
  void _drawProgressBarWithLabelsAboveOrBelow(Canvas canvas) {
    ...
    // current time label
    final labelDy = (isLabelBelow) ? size.width * 0.07 + _timeLabelPadding : 0.0;
    final leftLabelDx = _rightLabelSize.width * 3.7;
    final leftLabelOffset = Offset(leftLabelDx, labelDy);
    _leftTimeLabel().paint(canvas, leftLabelOffset);

    //Back slash
    final backSlashLabelDy = (isLabelBelow) ? size.width * 0.07 + _timeLabelPadding : 0.0;
    final backSlashLabelDx = _rightLabelSize.width * 5;
    final backSlashLabelOffset = Offset(backSlashLabelDx, backSlashLabelDy);
    middleBackSlash().paint(canvas, backSlashLabelOffset);

    // total or remaining time label
    final rightLabelDx = size.width - _rightLabelSize.width * 6;
